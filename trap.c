...
int getVictimIndex(bool try1, uint32 start,uint32 end){
...
void pagePlacement(struct Env *curenv, uint32 fault_va, uint32 victim){
	...
	if (returnOfReadEnvPage == E_PAGE_NOT_EXIST_IN_PF){
		if (isValidFaultVa(fault_va)){
			...
			panic("Error: invalid access page that's not exist in page file and belong to the stack");
		}
	}
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	else victim = curenv->page_last_WS_index;
