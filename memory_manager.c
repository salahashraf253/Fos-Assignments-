...
void clear_TLB_Cache(){
	tlbflush();
}

bool isValidVirtualAddress(uint32 va, uint32 virtual_address,uint32 size){
	/*check that the virtual address is in range of page file*/
	return (va >= virtual_address && va < virtual_address + size);
}
bool isEmptyEntry(struct Env* e, uint32 index){
	return env_page_ws_is_entry_empty(e,index);
}
...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32 physicalAddressOfPageTable=kheap_physical_address(pageTable);
	uint32 perm= PERM_PRESENT | PERM_USER | PERM_WRITEABLE;
	/*build the table entry then link it to the given directory*/
	ptr_page_directory[getDirectoryIndex(virtual_address)] =CONSTRUCT_ENTRY(physicalAddressOfPageTable,perm);
	uint32 *table_ptr;
	table_ptr=(uint32*)pageTable;	/*clear all entries (as it may contain garbage data)*/
...
// [12] freeMem
void freeAllPagees(struct Env* e,uint32 virtual_address,int noOfPages){
...
void freeResidentPagesInTheWorkingSet(struct Env* e,uint32 virtual_address,uint32 size){
	uint32 workingSetMaxSize = e->page_WS_max_size;
	struct WorkingSetElement workingSet[env_page_ws_get_size(e)];
	int ctr = 0;
	bool isEmptyEntry;
	uint32 tempVa;
	for(int i=0;i<workingSetMaxSize;i++){
		tempVa= env_page_ws_get_virtual_address(e,i);
		isEmptyEntry=env_page_ws_is_entry_empty(e,i);
		if(!isEmptyEntry && (tempVa >= virtual_address && tempVa < virtual_address + size)){
			unmap_frame(e->env_page_directory,(void*)tempVa);
		}
		else if(!isEmptyEntry){
			workingSet[(++ctr)-1] = e->ptr_pageWorkingSet[i];
		}
		env_page_ws_clear_entry(e,i);
	}
	int i=0;
	while(i<ctr){
		e->ptr_pageWorkingSet[i] = workingSet[i];
		i++;
	}
	e->page_last_WS_index = ctr % workingSetMaxSize;
}
void removeEmptyNonUsedPages(struct Env* e,uint32 virtual_address,uint32 noOfPages){

	int flag;
	for(int i=0;i<noOfPages;i++,virtual_address+=PAGE_SIZE){
		uint32 * page_table_ptr = NULL;
		get_page_table(e->env_page_directory,(void*)virtual_address,&page_table_ptr);
		if(page_table_ptr != NULL){
			flag=0;
			while(flag<1024){
				if(page_table_ptr[flag] != 0){
					flag=-1;
					break;
				}
				else flag++;
			...
			if(flag != -1){
				...
				e->env_page_directory[getDirectoryIndex(virtual_address)] = 0;
				clear_TLB_Cache();
			}
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//1. Free ALL pages of the given range from the Page File
	freeAllPagees(e,virtual_address,noOfPages);
	//2. Free ONLY pages that are resident in the working set from the memory
	freeResidentPagesInTheWorkingSet(e,virtual_address,size);
	//3. Removes ONLY the empty page tables (i.e. not used) (no pages are mapped in the table)
	removeEmptyNonUsedPages(e,virtual_address,noOfPages);
}
