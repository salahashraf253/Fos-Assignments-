...
void initializeQueue(int numOfLevels)
{
	for (uint32 i = 0; i < numOfLevels; i++)
		init_queue(&env_ready_queues[i]);		/*initialize them using init_queue()*/
}
void initializeQuantums(int numOfLevels, uint8 *quantumOfEachLevel)
{
	for (uint32 i = 0; i < numOfLevels; i++)
...
//	cprintf("Hello in sched_init_MLFQ\n");
	...
	initializeQueue(numOfLevels);
	initializeQuantums(numOfLevels,quantumOfEachLevel);
